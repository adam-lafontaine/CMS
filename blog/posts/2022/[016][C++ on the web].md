...
Emscripten works with many popular C++ libraries including SDL2.  Rather than image data being written to a window buffer, it will be written to the html canvas element.
...
Emscripten handles the framerate of the application so there is no need run an infinite loop in the main function and track the execution time of each frame.  We still need to define a function that executes in each frame with a means to exit if the application finishes.
