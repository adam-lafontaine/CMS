...
The code in this post is compiled and run on Ubuntu.  Lets start with a simple program that runs in the console and change it as we go.
...
This is the default webpage generated by Emscripten.  The console output from our program is written to a text area in the bottom section.  The top section is a canvas element where any generated graphics are displayed.

### Using SDL2

Emscripten works with many popular C++ libraries including SDL2.  Rather than image data being written to a window of a desktop application, it will be written to the html canvas element.
...
```cpp
bool init_canvas_buffer(CanvasBuffer& buffer, int width, int height)
{
...
Emscripten handles the framerate of the application so there is no need run an infinite loop in the main function and track the execution time of each frame.  We still need to define a function that executes in each frame with a means to exit when the application finishes.
